...
import { Check, Crosshair, Pencil, Plus, Trash2 } from "lucide-react";
...
const monthName = (month) => MONTH_NAMES[(month || 1) - 1];

// Yearly goals card for the year planner.
const YearlyGoals = ({
  year,
  categories = [],
  onChanged,
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getYearlyGoals(year);
      setGoals(data);
      onChanged?.(data.length, data.filter((goal) => goal.completed).length);
    } catch {
      setError("Unable to load yearly goals.");
    } finally {
      setLoading(false);
    }
  }, [year, onChanged]);
  ...
  const handleCreate = async (payload) => {
    ...
    try {
      const { goal } = await createYearlyGoal(payload);
  ...
  const handleUpdate = async (payload) => {
    ...
    try {
      const { goal } = await updateYearlyGoal(editing._id, payload);
      setGoals((previous) =>
  ...
  const handleToggle = async (goal) => {
    ...
    try {
      const { goal: updated } = await updateYearlyGoal(goal._id, { completed, progress });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteYearlyGoal(confirmDelete._id);
    ...
    <div className="goals-card">
      ...
      {error ? (
        ...
        <div className="goals-empty">
          <Crosshair size={22} />
          <p>No goals for {year} yet. Add one to shape the whole year.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="goal-body">
                ...
                <span className="goal-meta">
                  {goal.targetMonth && <em>{monthName(goal.targetMonth)}</em>}
                  {goal.category?.name && (
                    <span className={`category-tag cat-${goal.category?.color || "lavender"}`}>
